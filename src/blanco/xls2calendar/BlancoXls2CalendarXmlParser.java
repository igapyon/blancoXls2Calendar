	...
	public BlancoXls2CalendarStructure parseElementSheet(final BlancoXmlElement argElementSheet) {
		...
		for (int indexField = 0; indexField < listEvent.size(); indexField++) {
			...
			if (BlancoStringUtil.null2Blank(BlancoXmlBindingUtil.getTextContent(elementField, "ymd")).length() > 0) {
				...
				eventStructure.setDateBegin(BlancoXls2CalendarUtil.fromDate(date));
				System.out.println("begin:"+dateBeginPart.toString());
				System.out.println("end:"+dateBeginPart.toString());
				eventStructure.getDateBegin().set(Calendar.HOUR_OF_DAY, dateBeginPart.getHours());
				eventStructure.getDateBegin().set(Calendar.MINUTE, dateBeginPart.getMinutes());
				eventStructure.setDateEnd(BlancoXls2CalendarUtil.fromDate(date));
				eventStructure.getDateEnd().set(Calendar.HOUR_OF_DAY, dateEndPart.getHours());
				eventStructure.getDateEnd().set(Calendar.MINUTE, dateEndPart.getMinutes());
			}
